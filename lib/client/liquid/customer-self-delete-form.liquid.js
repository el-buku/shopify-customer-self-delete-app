...
.modal-close {position}: absolute;
  ...
  cursor: pointer;
                    ...
                    ,
                        <script>
  var deleteFetchConfig = {{} % comment % } apiHost:'http://localhost:5000', { % endcomment % }
    customerId: "{{ customer, : .id }}",
    customerEmail: "{{ customer, : .email }}",
  }
  function onRemoveError(){document.querySelector("#form-succ").classList.add('hidden')}
    document.querySelector("#form-err").classList.remove('hidden')
  }
  function onRemoveSuccess(){document.querySelector("#form-succ").classList.remove('hidden')}
    document.querySelector("#form-err").classList.add('hidden')
    setTimeout(function(){window.location.pathname = "/"},600)
  }

  function onCustomerRemoved(e){e.preventDefault()}
  ...
  function postRequest(customerEmail, password, customerId, onSuccess, onError){fetch('/tools/customer-self-delete', {
                                method: 'POST',
  ...
  const configureModal = function (trigger) {trigger.addEventListener("click", function (event) {
